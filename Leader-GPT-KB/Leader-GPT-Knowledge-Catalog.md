...
knowledge_catalog:
  ...
  purpose:
    ...
    - "区分项目背景类、Specs 方向类、阅读协议类、输出模板类与发布约束类文件。"
...
direction_files:
  - id: "current-specs-completion-direction"
    ...
    use_when:
      ...
      - "判断 Specs 是否应推进到 requirements.md / design.md / tasks.md。"
...
project_knowledge_files:
  ...
  - id: "KB-file-unit-intros"
    file: "KB-file-unit-intros.yaml"
    type: "file_unit_intro"
    priority: "medium"
    use_when:
      - "需要快速判断某个知识源文件的用途。"
      - "需要决定某个文件在任务包中应作为 R1、R2 还是 R3。"
      - "需要避免误用某个文件。"
    do_not_use_for:
      - "替代模块知识图谱。"
      - "替代源文件正文蒸馏内容。"
...
specs_definition_files:
  - id: "kiro-specs-definition"
    ...
    priority: "high"
    use_when:
      - "需要理解 Kiro Specs 的 requirements.md / design.md / tasks.md 三层结构。"
      - "需要判断本轮目标文件属于哪个 Spec 层。"
      - "需要写 KiroPrompt-GPT Task Packet 的 Target Spec。"
...
leader_output_templates:
  - id: "kiro-prompt-task-packet-template"
    file: "kiro-prompt-task-packet-template.md"
    ...
    use_when:
      ...
      - "说明本轮目标 Spec、任务意图、范围边界、Reading Scope、扩读条件和预期输出。"
    do_not_use_for:
      - "生成 Review-GPT 审查意图包。"
      - "生成 Round Meta JSON。"
  ...
  - id: "review-intent-packet-template"
    ...
    use_when:
      ...
      - "说明本轮产物的 Leader Intent、Review Target、Review Focus 和 Do Not Overreview。"
    do_not_use_for:
...
release_format_files:
  - id: "leader-round-output-envelope-standard"
    file: "leader-round-output-envelope-standard.md"
    type: "output_envelope_standard"
    priority: "high"
    use_when:
      - "将 KiroPrompt-GPT Task Packet、Review Intent Packet、Round Meta 组合成一轮完整输出。"
      - "确认三个强 marker 区块的顺序。"
  ...
  - id: "leader-output-release-gate"
    file: "leader-output-release-gate.yaml"
    type: "release_gate"
...
default_execution_order:
  - step: 1
    ...
    goal: "识别当前用户输入要求是新建、继续推进，还是大修返工。"
  ...
  - step: 3
    ...
    goal: "确定 action 为 Create 或 MajorRevision。"
  ...
  - step: 8
    ...
    goal: "按固定强 marker 顺序组合三个区块。"

  - step: 9
    node: "validate_before_release"
    goal: "发布前检查格式、边界和必填内容。"
```
